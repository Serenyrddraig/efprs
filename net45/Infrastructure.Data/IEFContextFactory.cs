...
namespace Infrastructure.Data
{
    ...
    /// 
    ///    private static void RegisterContextFactory(IUnityContainer container, string ConnectionString, IEnumerable<string> assemblies)
    ///    {
    ///        var bldr = new DbContextBuilder<DbContext>(ConnectionString, assemblies.ToArray(),
    ///                                                       false, false);
    ///        var model = bldr.CompiledModel; // Force model compile now instead of on first access
    ///        container.RegisterInstance(typeof(IEFContextFactory<DbContext>), conString,
    ///            bldr as IEFContextFactory<DbContext>, new ContainerControlledLifetimeManager());
    ///    }
    /// 
    /// Repository construction could then access the factory for a repository unique context
    /// 
    ///     var factory = container.Resolve<IEFContextFactory<DbContext>>(ConnectionString);
